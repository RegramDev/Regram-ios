...
final class UniversalVideoGalleryItemNode: ZoomableContentGalleryItemNode {
    ...
    override func containerLayoutUpdated(_ layout: ContainerViewLayout, navigationBarHeight: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        // MARK: Regram — playback moved into the footer, on its own row between the scrubber and the
        // action buttons (see ChatItemGalleryFooterContentNode). This centred overlay covered the
        // middle of the frame, so a tap meant to show/hide the chrome hit play/pause or a 15-second
        // jump instead. Hidden rather than deleted: upstream's seek and animation plumbing stays
        // intact, and one flag brings it back.
        playbackControlsIsVisible = false
    ...
    override func maybePerformActionForSwipeDismiss() -> Bool {
        if #available(iOS 15.0, *) {
            if RGSimpleSettings.shared.videoPIPSwipeDirection != RGSimpleSettings.VideoPIPSwipeDirection.up.rawValue {
                return false
            }
        }
    ...
    private typealias RGContextMenuItems = (items: [ContextMenuItem], topItems: [ContextMenuItem])

    private func contextMenuMainItems(isSettings: Bool, dismiss: @escaping () -> Void) -> Signal<RGContextMenuItems, NoError> {
        ...
        func mapMenu(values: (MediaPlayerStatus?, EnginePeer?, (current: Int, preferred: UniversalVideoContentVideoQuality, available: [Int])?)) -> RGContextMenuItems {
    ...
    func settingsButtonPressed(sourceView: UIView) {
        // MARK: Regram — the speed/quality button lives in the top control strip, so the menu must
        // drop *below* it. `actionsOnTop: true` (the upstream value, from when this button sat at the
        // bottom) anchors the menu above the button, and the reference-source layout has no status-bar
        // clamp — so the top rows (Normal, 0.5x) land off the top edge and can't be tapped.
        self.openMoreMenu(sourceView: sourceView, gesture: nil, adMessage: nil, isSettings: true, actionsOnTop: false)
    }
