        ...
        if #available(iOS 13.0, *) {
            SFSpeechRecognizer.requestAuthorization { status in
                Queue.mainQueue().async {
                    switch status {
                    case .notDetermined:
                        RGLogger.shared.log("LocalTranscription", "Authorization status: notDetermined")
                        subscriber.putNext(nil)
                        subscriber.putCompletion()
                    case .restricted:
                        RGLogger.shared.log("LocalTranscription", "Authorization status: restricted")
                        subscriber.putNext(nil)
                        subscriber.putCompletion()
                    case .denied:
                        RGLogger.shared.log("LocalTranscription", "Authorization status: denied")
                        subscriber.putNext(nil)
                        subscriber.putCompletion()
                    case .authorized:
                        ...
                        if let sharedRecognizer = sharedRecognizers[locale] as? SFSpeechRecognizer {
                            ...
                            guard let speechRecognizerValue = SFSpeechRecognizer(locale: Locale(identifier: locale)), speechRecognizerValue.isAvailable else {
                                RGLogger.shared.log("LocalTranscription", "Recognizer not available for locale: \(locale)")
                                subscriber.putNext(nil)
                                subscriber.putCompletion()
                        ...
                        let task = speechRecognizer.recognitionTask(with: request, resultHandler: { result, error in
                            if let result = result {
                                ...
                                if result.isFinal {
                                    RGLogger.shared.log("LocalTranscription", "Transcription finalized. locale: \(locale), segments: \(segmentsCount), confidence: \(confidence)")
                                    subscriber.putCompletion()
                                }
                            } else {
                                RGLogger.shared.log("LocalTranscription", "Transcription failed. locale: \(locale), error: \(String(describing: error))")
                    ...
                    @unknown default:
                        RGLogger.shared.log("LocalTranscription", "Unknown authorization status")
                        subscriber.putNext(nil)
                        subscriber.putCompletion()
                    }
                }
...
//    }
    ...
    |> map { results -> LocallyTranscribedAudio? in
        ...
        guard let bestLocale = accumulatedTranscription.max(by: { $0.value.confidence < $1.value.confidence }) else {
            RGLogger.shared.log("LocalTranscription", "No valid transcription results found")
            return nil
        }
