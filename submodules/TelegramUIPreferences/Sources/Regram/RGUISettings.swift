...
public struct RGUISettings: Equatable, Codable {
    public var hideStories: Bool
    public var showProfileId: Bool
    public var warnOnStoriesOpen: Bool
    public var sendWithReturnKey: Bool
    
    public static var `default`: RGUISettings {
        return RGUISettings(hideStories: false, showProfileId: true, warnOnStoriesOpen: false, sendWithReturnKey: false)
    }
...
public func updateRGUISettings(engine: TelegramEngine, _ f: @escaping (RGUISettings) -> RGUISettings) -> Signal<Never, NoError> {
    return engine.preferences.update(id: ApplicationSpecificPreferencesKeys.RGUISettings, { entry in
        let currentSettings: RGUISettings
        if let entry = entry?.get(RGUISettings.self) {
