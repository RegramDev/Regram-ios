                        ...
                        if let image = image {
                            if let info = info, let degraded = info[PHImageResultIsDegradedKey], (degraded as AnyObject).boolValue!{
                                ...
                                case .none, .jpeg:
                                    ...
                                    if let scaledImage = scaledImage, let data = compressImageToJPEG(scaledImage, quality: Float(RGSimpleSettings.shared.outgoingPhotoQuality) / 100.0, tempFilePath: tempFile.path) {
    #if DEBUG
                                ...
                                case .jxl:
                                    if let scaledImage = scaledImage, let data = compressImageToJPEGXL(scaledImage, quality: Int(RGSimpleSettings.shared.outgoingPhotoQuality)) {
    #if DEBUG
