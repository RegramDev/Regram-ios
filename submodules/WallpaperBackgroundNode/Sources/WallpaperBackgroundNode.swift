...
//                        needsCleanBackground = false
    ...
    public func updateLayout(size: CGSize, displayMode: WallpaperDisplayMode, transition: ContainedViewLayoutTransition) {
        ...
        if isFirstLayout && !self.frame.isEmpty {
            self.updateScale()
            /* MARK: Regram */ if RGSimpleSettings.shared.isNYEnabled && self.NYNode == nil { let nYNode = WallpaperNYNode(); self.addSubnode(nYNode); self.NYNode = nYNode }
        }
        /* MARK: Regram */ self.NYNode?.frame = CGRect(origin: CGPoint(), size: size); self.NYNode?.updateLayout(size: size)
    }
