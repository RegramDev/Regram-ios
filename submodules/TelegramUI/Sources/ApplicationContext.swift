...
final class AuthorizedApplicationContext {
    ...
    init(sharedApplicationContext: SharedApplicationContext, mainWindow: Window1, context: AccountContextImpl, accountManager: AccountManager<TelegramAccountManagerTypes>, showContactsTab: Bool, showCallsTab: Bool, reinitializedNotificationSettings: @escaping () -> Void) {
        ...
        if self.rootController.rootTabController == nil {
            self.rootController.addRootControllers(hidePhoneInSettings: RGSimpleSettings.shared.hidePhoneInSettings, showContactsTab: self.showContactsTab, showCallsTab: self.showCallsTab)
        }
        ...
        if #available(iOS 10.0, *) {
            ...
            |> deliverOnMainQueue).start(next: { [weak self] required, splitTest, position, contactsPermissionWarningNotice, notificationsPermissionWarningNotice, cellularDataPermissionWarningNotice in
                ...
                var contactsTimestamp = contactsPermissionWarningNotice.value.flatMap({ ApplicationSpecificNotice.getTimestampValue($0) })
                var notificationsTimestamp = notificationsPermissionWarningNotice.value.flatMap({ ApplicationSpecificNotice.getTimestampValue($0) })
                ...
                // MARK: Regram — the "already asked" notice (value 0) lives in the shared account
                // manager and therefore outlives the app's TCC identity: after a reinstall under a
                // different bundle id, or a container carried over from another build, iOS reports the
                // permission as never requested while the notice still says it was handled, so the
                // prompt is skipped forever and the user is left with no notifications and no contacts.
                // The OS is authoritative here — if it says not-determined, the notice is stale.
                if contactsTimestamp == 0, case .requestable = required.0.status {
                    contactsTimestamp = nil
                }
                if notificationsTimestamp == 0, case .requestable = required.1.status {
                    notificationsTimestamp = nil
                }
