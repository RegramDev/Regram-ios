            ...
            if case let .peer(peerView) = self.chatLocationInfoData, let peerId = peerId {
                ...
                self.peerDisposable = combineLatest(
                    ...
                    if case .standard(.default) = mode, let channel = renderedPeer?.chatMainPeer as? TelegramChannel, case .broadcast = channel.info {
                        ...
                        if strongSelf.nextChannelToReadDisposable == nil, let peerId = chatLocation.peerId, let customChatNavigationStack, !RGSimpleSettings.shared.disableScrollToNextChannel {
                        ...
                        } else if isRegularChat, strongSelf.nextChannelToReadDisposable == nil, !RGSimpleSettings.shared.disableScrollToNextChannel {
                            //TODO:loc optimize
                ...
                |> deliverOnMainQueue).startStrict(next: { [weak self] peerView, messageAndTopic, savedMessagesPeer, onlineMemberCount, hasScheduledMessages, hasSearchTags, hasSavedChats, isPremiumRequiredForMessaging, managingBot, globalPrivacySettings, canStopIncomingStreamingMessage in
                    ...
                    if let savedMessagesPeerId {
                        ...
                        if let replyThreadId, let channel = renderedPeer?.peer as? TelegramChannel, channel.isForumOrMonoForum, strongSelf.nextChannelToReadDisposable == nil, !RGSimpleSettings.shared.disableScrollToNextTopic {
                            strongSelf.nextChannelToReadDisposable = (combineLatest(queue: .mainQueue(),
            ...
            if let peerId = chatLocation.peerId {
                ...
                if case .standard(.previewing) = mode {
                    ...
                    self.translationStateDisposable = (combineLatest(
                        ...
                        if (isPremium || maybeSuggestPremium || hasAutoTranslate || true /* MARK: Regram */) && !isHidden {
