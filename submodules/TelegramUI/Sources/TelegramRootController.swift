                ...
                if let mediaResult = result.media {
                    ...
                    case let .video(content, firstFrameImage, values, duration, dimensions):
                        ...
                        if let valuesData = try? JSONEncoder().encode(values) {
                            ...
                            let imageData = firstFrameImage.flatMap { compressImageToJPEG($0, quality: Float(RGSimpleSettings.shared.outgoingPhotoQuality) / 100.0, tempFilePath: tempFile.path) }
