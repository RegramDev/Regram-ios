...
private final class StoryContainerScreenComponent: Component {
    ...
    final class View: UIView, UIGestureRecognizerDelegate {
        ...
        private var requestedDisplayStoriesWarning: Bool = RGSimpleSettings.shared.warnOnStoriesOpen // MARK: Regram
        ...
        func update(component: StoryContainerScreenComponent, availableSize: CGSize, state: EmptyComponentState, environment: Environment<ViewControllerComponentContainer.Environment>, transition: ComponentTransition) -> CGSize {
            ...
            if self.component == nil {
                ...
                // MARK: Regram
                self.requestedDisplayStoriesWarning = RGSimpleSettings.shared.warnOnStoriesOpen
            ...
            if self.component?.content !== component.content {
                ...
                let contentUpdated: (StoryContainerScreenComponent) -> Void = { [weak self] component in
                    ...
                    if let slice = stateValue?.slice {
                        ...
                        // TODO(regram): Show warning on each new peerId story
            ...
            if self.isDisplayingStoriesWarning {
                let _ = self.storiesWarning.update(
                    ...
                    component: AnyComponent(
                        RGStoryWarningComponent(
                            context: component.context,
                            theme: environment.theme,
                            strings: environment.strings,
                            peer: component.content.stateValue?.slice?.peer,
                            isInStealthMode: stealthModeTimeout != nil || RGSimpleSettings.shared.isStealthModeEnabled,
                            action: { [weak self] in
                                self?.isDisplayingStoriesWarning = false
                                self?.state?.updated(transition: .immediate)
                                if let view = self?.storiesWarning.view as? RGStoryWarningComponent.View {
