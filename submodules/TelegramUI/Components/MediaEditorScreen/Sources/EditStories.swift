                ...
                if repost {
                    ...
                    if let mediaResult = result.media {
                        ...
                        case let .image(image, dimensions):
                            ...
                            if let imageData = compressImageToJPEG(image, quality: Float(RGSimpleSettings.shared.outgoingPhotoQuality) / 100.0, tempFilePath: tempFile.path) {
                        ...
                        case let .video(content, firstFrameImage, values, duration, dimensions):
                            ...
                            if let valuesData = try? JSONEncoder().encode(values) {
                                ...
                                let firstFrameImageData = firstFrameImage.flatMap { compressImageToJPEG($0, quality: Float(RGSimpleSettings.shared.outgoingPhotoQuality) / 100.0, tempFilePath: tempFile.path) }
