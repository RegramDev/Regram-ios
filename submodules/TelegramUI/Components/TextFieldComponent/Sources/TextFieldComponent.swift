...
public final class TextFieldComponent: Component {
    ...
    public final class View: UIView, UIScrollViewDelegate, ChatInputTextNodeDelegate {
        ...
        // MARK: Regram
        var rgToolbarActionObserver: NSObjectProtocol? = nil
        ...
        override init(frame: CGRect) {
            ...
            // MARK: Regram
            self.rgToolbarActionObserver = NotificationCenter.default.addObserver(forName: Notification.Name("sgToolbarAction"), object: nil, queue: .main, using: { [weak self] notification in
                    guard let self = self else { return }
                    if let action = notification.userInfo?["action"] as? String {
                        self.rgToolbarAction(action)
                    }
            })
        }
        ...
        // MARK: Regram
        deinit {
            if let rgToolbarActionObserver = self.rgToolbarActionObserver {
                NotificationCenter.default.removeObserver(rgToolbarActionObserver)
            }
        }
...
extension TextFieldComponent.View {
    ...
    func rgToolbarAction(_ action: String) {
        switch action {
            case "quote":
                self.rgSelectLastWordIfIdle()
                self.toggleAttribute(key: ChatTextInputAttributes.block, value: ChatTextInputTextQuoteAttribute(kind: .quote, isCollapsed: false))
            case "spoiler":
                self.rgSelectLastWordIfIdle()
                self.toggleAttribute(key: ChatTextInputAttributes.spoiler)
