...
final class PeerInfoScreenNode: ViewControllerTracingNode, PeerInfoScreenNodeProtocol, ASScrollViewDelegate {
    ...
    private(set) var showProfileId: Bool = RGSimpleSettings.shared.showProfileId // MARK: Regram
    ...
    init(
        hidePhoneInSettings: Bool /* MARK: Regram */, controller: PeerInfoScreenImpl,
        ...
        self.headerNode.navigationButtonContainer.performAction = { [weak self] key, source, gesture in
            ...
            case .done, .cancel:
                ...
                (strongSelf.controller?.parent as? TabBarController)?.updateIsTabBarHidden(RGSimpleSettings.shared.hideTabBar ? true : false, transition: .animated(duration: 0.3, curve: .linear))
            case .select:
        ...
        self.dataDisposable = combineLatest(
            ...
            // MARK: Regram
            strongSelf.showProfileId = RGSimpleSettings.shared.showProfileId
            //
    ...
    public func openParticipantsSection(section: PeerInfoParticipantsSection) { // MARK: Regram
        guard let data = self.data, let peer = data.peer else {
            return
        }
        switch section {
    ...
    func deactivateSearch() {
        ...
        if self.isSettings {
            (self.controller?.parent as? TabBarController)?.updateIsTabBarHidden(RGSimpleSettings.shared.hideTabBar ? true : false, transition: .animated(duration: 0.4, curve: .spring))
        ...
        if let (layout, navigationHeight) = self.validLayout {
            ...
            if self.state.isEditing {
                ...
                if self.isSettings {
                    ...
                    if RGSimpleSettings.shared.hideTabBar { leftNavigationButtons.append(PeerInfoHeaderNavigationButtonSpec(key: .back, isForExpandedView: false)) }
...
public final class PeerInfoScreenImpl: ViewController, PeerInfoScreen, KeyShortcutResponder {
    ...
    public init(
        hidePhoneInSettings: Bool = RGSimpleSettings.defaultValues[RGSimpleSettings.Keys.hidePhoneInSettings.rawValue] as! Bool,
        context: AccountContext,
    ...
    override public func tabBarItemContextAction(sourceView: ContextExtractedContentContainingView, gesture: ContextGesture) {
        ...
        // MARK: Regram
        #if DEBUG
        items.append(.action(ContextMenuActionItem(text: "Regram Debug", icon: { theme in
            return generateTintedImage(image: nil, color: theme.contextMenu.primaryColor)
        }, action: { [weak self] _, f in
            guard let self = self else {
                return
            }
            self.push(rgDebugController(context: self.context))
