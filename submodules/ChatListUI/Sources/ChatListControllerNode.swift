...
public final class ChatListContainerNode: ASDisplayNode, ASGestureRecognizerDelegate {
        ...
        openArchiveSettings: @escaping () -> Void)
    {
        ...
        let panRecognizer = InteractiveTransitionGestureRecognizer(target: self, action: #selector(self.panGesture(_:)), allowedDirections: { [weak self] _ in // MARK: Regram
            guard let self, self.availableFilters.count > 1 || (self.controller?.isStoryPostingAvailable == true && !(self.context.sharedContext.callManager?.hasActiveCall ?? false) && !RGSimpleSettings.shared.disableSwipeToRecordStory) else {
                return []
            }
        ...
        }, edgeWidth: RGSimpleSettings.shared.disableChatSwipeOptions ? .widthMultiplier(factor: 1.0 / 6.0, min: 0.0, max: 0.0) : .widthMultiplier(factor: 1.0 / 6.0, min: 22.0, max: 80.0))
...
final class ChatListControllerNode: ASDisplayNode, ASGestureRecognizerDelegate {
    ...
    let rgFoldersView = ComponentView<Empty>()
    ...
    private func updateNavigationBar(layout: ContainerViewLayout, deferScrollApplication: Bool, transition: ComponentTransition) -> (tabs: AnyComponent<Empty>?, navigationHeight: CGFloat, storiesInset: CGFloat) {
            ...
            panels.append(HeaderPanelContainerComponent.Panel(
                ...
                component: AnyComponent(ChatListHeaderNoticeComponent(
                    ...
                    activateAction: { [weak self] notice in
                        ...
                        switch notice {
                        case let .rgUrl(id, _, _, url, needAuth, permanent):
                            self.effectiveContainerNode.currentItemNode.interaction?.openRGAnnouncement(id, url, needAuth, permanent)
                        case .clearStorage:
        ...
        var tabs: AnyComponent<Empty>? // MARK: Regram
        if self.controller?.tabContainerData != nil || !panels.isEmpty {
            if let tabContainerData = self.controller?.tabContainerData, tabContainerData.0.count > 1 {
                ...
                tabs = AnyComponent(HorizontalTabsComponent(
                    ...
                    tabs: tabContainerData.0.map { entry -> HorizontalTabsComponent.Tab in
                        ...
                        case .all:
                            id = Int32.min
                            title = HorizontalTabsComponent.Tab.Title(text: rgUseShortAllChatsTitle(true) ? self.presentationData.strings.ChatList_Tabs_All : self.presentationData.strings.ChatList_Tabs_AllChats, entities: [], enableAnimations: false)
                            isMainTab = true
            ...
            navigationHeaderPanels = AnyComponent(HeaderPanelContainerComponent(
                theme: self.presentationData.theme,
                tabs: (self.controller?.tabContainerData?.1 ?? false) ? nil : tabs, // MARK: Regram
                panels: panels
            ))
        }
    ...
    func containerLayoutUpdated(_ layout: ContainerViewLayout, navigationBarHeight: CGFloat, visualNavigationHeight: CGFloat, cleanNavigationBarHeight: CGFloat, storiesInset: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        // MARK: Regram
        let rgComponentTransition = ComponentTransition(transition)
        let rgDisplayTabsAtBottom = self.controller?.tabContainerData?.1 ?? false
        let rgShouldDisplayBottomFolders = rgDisplayTabsAtBottom && self.isSearchDisplayControllerActive == nil
        let rgFoldersSize = self.rgFoldersView.update(
            transition: rgComponentTransition,
        ...
        // MARK: Regram
        if rgShouldDisplayBottomFolders && rgFoldersSize.height > 0.0 {
            insets.bottom += rgFoldersSize.height + 16.0 + 8.0
        }
        ...
        // MARK: Regram
        if let rgFoldersView = self.rgFoldersView.view as? HeaderPanelContainerComponent.View {
            if rgShouldDisplayBottomFolders && rgFoldersSize.height > 0.0 {
                rgFoldersView.layer.removeAnimation(forKey: "opacity")
                rgFoldersView.alpha = 1.0
                if rgFoldersView.superview == nil {
                    self.view.addSubview(rgFoldersView)
                    if transition.isAnimated {
                        rgFoldersView.layer.animateAlpha(from: 0.0, to: 1.0, duration: 0.23)
                    }
                }
...
//                }
                //
                // TODO(regram):
                rgComponentTransition.setFrame(view: rgFoldersView, frame: CGRect(origin: CGPoint(x: 0.0, y: layout.size.height - bottomInset - rgFoldersSize.height - 16.0), size: rgFoldersSize))
            } else {
                rgFoldersView.layer.animateAlpha(from: 1.0, to: 0.0, duration: 0.2, removeOnCompletion: false, completion: { [weak rgFoldersView] completed in
                    if completed {
                        rgFoldersView?.removeFromSuperview()
                    }
                })
            }
        }
