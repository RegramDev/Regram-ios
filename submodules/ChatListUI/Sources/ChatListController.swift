...
public class ChatListControllerImpl: TelegramBaseController, ChatListController {
        ...
        if case .chatList(.root) = self.location {
            self.chatListDisplayNode.mainContainerNode.currentItemFilterUpdated = { [weak self] filter, fraction, transition, force in
                ...
                if fraction.isZero {
                    ...
                    if RGSimpleSettings.shared.lastAccountFolders[accountId] != switchingToFilterId {
                        RGSimpleSettings.shared.lastAccountFolders[accountId] = switchingToFilterId
                    }
                }
    ...
    override public func loadDisplayNode() {
        ...
        if case .chatList = self.location {
            ...
            if self.previewing {
                ...
                // MARK: Regram
                let hideStoriesSignal = rgSimpleSettingsBoolSignal(.hideStories, defaultValue: false)
                    ...
                    if let strongSelf = self {
                        if !tooltipValue {
                            ...
                            if hasPasscode {
                                ...
                                let tooltipController = TooltipController(content: .text(strongSelf.presentationData.strings.DialogList_PasscodeLockHelp.replacingOccurrences(of: "Telegram", with: "Regram") /* MARK: Regram */), baseFontSize: strongSelf.presentationData.listsFontSize.baseDisplaySize, dismissByTapOutside: true)
    ...
    private func reloadFilters(firstUpdate: (() -> Void)? = nil) {
        ...
        |> deliverOnMainQueue).startStrict(next: { [weak self] displayTabsAtBottom, countAndFilterItems, peerView, limits in
            ...
            // MARK: Regram
            if !strongSelf.initializedFilters && RGSimpleSettings.shared.rememberLastFolder {
                if let lastFolder = RGSimpleSettings.shared.lastAccountFolders["\(strongSelf.context.account.peerId.id._internalGetInt64Value())"]{
