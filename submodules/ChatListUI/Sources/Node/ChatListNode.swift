...
public final class ChatListNodeInteraction {
    ...
    let openRGAnnouncement: (String, String, Bool, Bool) -> Void
    ...
    public init(
        ...
        // MARK: Regram
        openRGAnnouncement: @escaping (String, String, Bool, Bool) -> Void = { _, _, _, _ in },
        ...
        self.openRGAnnouncement = openRGAnnouncement
...
public final class ChatListNode: ListViewImpl {
    ...
    public init(getNavigationController: (() -> NavigationController?)? = nil, context: AccountContext, location: ChatListControllerLocation, chatListFilter: ChatListFilter? = nil, previewing: Bool, fillPreloadItems: Bool, mode: ChatListNodeMode, isPeerEnabled: ((EnginePeer) -> Bool)? = nil, theme: PresentationTheme, fontSize: PresentationFontSize, strings: PresentationStrings, dateTimeFormat: PresentationDateTimeFormat, nameSortOrder: PresentationPersonNameOrder, nameDisplayOrder: PresentationPersonNameOrder, animationCache: AnimationCache, animationRenderer: MultiAnimationRenderer, disableAnimations: Bool, isInlineMode: Bool, autoSetReady: Bool, isMainTab: Bool?) {
        ...
        }, openRGAnnouncement: { [weak self] announcementId, url, needAuth, permanent in
            if let strongSelf = self {
                if needAuth {
                    let _ = (getRGSettingsURL(context: strongSelf.context, url: url)
                ...
                if !permanent {
                    Queue.mainQueue().after(0.6) { [weak self] in
                        if let strongSelf = self {
                            dismissRGProvidedSuggestion(suggestionId: announcementId)
                            postRGWebSettingsInteractivelly(context: strongSelf.context, data: ["skip_announcement_id": announcementId])
                        }
                    }
                }
            }
        ...
        // MARK: Regram — a chat list item only carries its top message, so once that message is
