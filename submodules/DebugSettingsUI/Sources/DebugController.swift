...
private enum DebugControllerEntry: ItemListNodeEntry {
    case RGDebug(PresentationTheme)
    case sendRGLogs(PresentationTheme)
    ...
    var section: ItemListSectionId {
        switch self {
        case .sendRGLogs, .RGDebug:
            return DebugControllerSection.regram.rawValue
    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        ...
        switch self {
        case .RGDebug:
            return ItemListDisclosureItem(presentationData: presentationData, title: "Regram Debug", label: "", sectionId: self.section, style: .blocks, action: {
                guard let context = arguments.context else {
                    return
                }
                arguments.pushController(rgDebugController(context: context))
            })
        case .testStickerImport:
        ...
        case .sendOneLog, .sendRGLogs:
            ...
            if case .sendRGLogs(_) = self {
                title = "Send Regram Logs"
                logCollectionSignal = RGLogger.shared.collectLogs()
                fileName = "Log-iOS-Regram.txt"
                appName = "Regram"
            }
...
private func debugControllerEntries(context: AccountContext?, sharedContext: SharedAccountContext, presentationData: PresentationData, loggingSettings: LoggingSettings, mediaInputSettings: MediaInputSettings, experimentalSettings: ExperimentalUISettings, networkSettings: NetworkSettings?, hasLegacyAppData: Bool, useBetaFeatures: Bool) -> [DebugControllerEntry] {
    ...
    // MARK: Regram
    entries.append(.RGDebug(presentationData.theme))
    entries.append(.sendRGLogs(presentationData.theme))
