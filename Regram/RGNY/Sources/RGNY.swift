...
class WallpaperNYNode: ASDisplayNode {
    ...
    func updateLayout(size: CGSize) {
        if self.emitterLayer == nil {
            ...
            switch RGSimpleSettings.shared.nyStyle {
                case RGSimpleSettings.NYStyle.lightning.rawValue:
                    // cell1.contents = generateTintedImage(image: UIImage(bundleImageName: "RegramContextMenu"), color: .white)
                    if let image = UIImage(bundleImageName: "RegramContextMenu") {
