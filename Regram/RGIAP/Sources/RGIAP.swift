...
public extension Notification.Name {
    static let RGIAPHelperPurchaseNotification = Notification.Name("SGIAPPurchaseNotification")
    static let RGIAPHelperErrorNotification = Notification.Name("SGIAPErrorNotification")
    static let RGIAPHelperProductsUpdatedNotification = Notification.Name("SGIAPProductsUpdatedNotification")
    static let RGIAPHelperValidationErrorNotification = Notification.Name("SGIAPValidationErrorNotification")
}

public final class RGIAPManager: NSObject {
    private var productRequest: SKProductsRequest?
    private var productsRequestCompletion: (([SKProduct]) -> Void)?
    private var purchaseCompletion: ((Bool, Error?) -> Void)?
    
    public private(set) var availableProducts: [RGProduct] = []
    private var finishedSuccessfulTransactions = Set<String>()
    private var onRestoreCompletion: (() -> Void)?
    
    public final class RGProduct: Equatable {
        ...
        public static func ==(lhs: RGProduct, rhs: RGProduct) -> Bool {
            if lhs.id != rhs.id {
                return false
            }
    ...
    public func buyProduct(_ product: SKProduct) {
        RGLogger.shared.log("SGIAP", "Buying \(product.productIdentifier)...")
        let payment = SKPayment(product: product)
        SKPaymentQueue.default().add(payment)
    }
    
    private func requestProducts() {
        RGLogger.shared.log("SGIAP", "Requesting products for \(RG_CONFIG.iaps.count) ids...")
        let productRequest = SKProductsRequest(productIdentifiers: Set(RG_CONFIG.iaps))
    ...
    public func restorePurchases(completion: @escaping () -> Void) {
        RGLogger.shared.log("SGIAP", "Restoring purchases...")
...
extension RGIAPManager: SKProductsRequestDelegate {
    public func productsRequest(_ request: SKProductsRequest, didReceive response: SKProductsResponse) {
        self.productRequest = nil
        
        DispatchQueue.main.async {
            let products = response.products
            RGLogger.shared.log("SGIAP", "Received products (\(products.count)): \(products.map({ $0.productIdentifier }).joined(separator: ", "))")
            let currentlyAvailableProducts = self.availableProducts
            self.availableProducts = products.map({ RGProduct(skProduct: $0) })
            if currentlyAvailableProducts != self.availableProducts {
                NotificationCenter.default.post(name: .RGIAPHelperProductsUpdatedNotification, object: nil)
            }
        }
    }
    
    public func request(_ request: SKRequest, didFailWithError error: Error) {
        RGLogger.shared.log("SGIAP", "Failed to load list of products. Error \(error.localizedDescription)")
        self.productRequest = nil
    }
}

extension RGIAPManager: SKPaymentTransactionObserver {
    public func paymentQueue(_ queue: SKPaymentQueue, updatedTransactions transactions: [SKPaymentTransaction]) {
        RGLogger.shared.log("SGIAP", "paymentQueue transactions \(transactions.count)")
        var purchaceTransactions: [SKPaymentTransaction] = []
        for transaction in transactions {
            RGLogger.shared.log("SGIAP", "Transaction \(transaction.transactionIdentifier ?? "nil") state for product \(transaction.payment.productIdentifier): \(transaction.transactionState.description)")
            switch transaction.transactionState {
                ...
                case .failed:
                    ...
                    if let transactionError = transaction.error as NSError?,
                        ...
                        RGLogger.shared.log("SGIAP", "Transaction Error [\(transaction.transactionIdentifier ?? "nil")]: \(localizedDescription)")
                    }
                    RGLogger.shared.log("SGIAP", "Sending SGIAPHelperErrorNotification for \(transaction.transactionIdentifier ?? "nil")")
                    NotificationCenter.default.post(name: .RGIAPHelperErrorNotification, object: transaction, userInfo: ["localizedError": localizedError])
                default:
                    RGLogger.shared.log("SGIAP", "Unknown transaction \(transaction.transactionIdentifier ?? "nil") state \(transaction.transactionState). Finishing transaction.")
                    SKPaymentQueue.default().finishTransaction(transaction)
            }
        }
        
        if !purchaceTransactions.isEmpty {
            RGLogger.shared.log("SGIAP", "Sending SGIAPHelperPurchaseNotification for \(purchaceTransactions.map({ $0.transactionIdentifier ?? "nil" }).joined(separator: ", "))")
            NotificationCenter.default.post(name: .RGIAPHelperPurchaseNotification, object: purchaceTransactions)
        }
    }
    
    public func paymentQueueRestoreCompletedTransactionsFinished(_ queue: SKPaymentQueue) {
        RGLogger.shared.log("SGIAP", "Transactions restored")
...
public func getPurchaceReceiptData() -> Data? {
    ...
    if let appStoreReceiptURL = Bundle.main.appStoreReceiptURL, FileManager.default.fileExists(atPath: appStoreReceiptURL.path) {
        do {
            ...
            RGLogger.shared.log("SGIAP", "Couldn't read receipt data with error: \(error.localizedDescription)")
        }
    } else {
        RGLogger.shared.log("SGIAP", "Couldn't find receipt path")
    }
    return receiptData
}
