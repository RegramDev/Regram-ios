...
public func rgPayWallController(statusSignal: Signal<Int64, NoError>, replacementController: ViewController, presentationData: PresentationData? = nil, RGIAPManager: RGIAPManager, openUrl: @escaping (String, Bool) -> Void /* url, forceExternal */, paymentsEnabled: Bool, canBuyInBeta: Bool, openAppStorePage: @escaping () -> Void, proSupportUrl: String?) -> ViewController {
    ...
    let swiftUIView = RGSwiftUIView<RGPayWallView>(
        legacyController: legacyController,
        content: {
            RGPayWallView(wrapperController: legacyController, replacementController: replacementController, RGIAP: RGIAPManager, statusSignal: statusSignal, openUrl: openUrl, openAppStorePage: openAppStorePage, paymentsEnabled: paymentsEnabled, canBuyInBeta: canBuyInBeta, proSupportUrl: proSupportUrl)
        }
    )
...
struct RGPayWallFeatureDetails: View {
    
    let dismissAction: () -> Void
    var bottomOffset: CGFloat = 0.0
    let contentHeight: CGFloat = 690.0
    let features: [RGProFeature]
    
    @State var shownFeature: RGProFeatureId?
...
struct RGProFeatureView: View {
    let feature: RGProFeature
...
struct RGProFeature: Identifiable {
    
    let id: RGProFeatureId
    let title: String
    let subtitle: String
...
struct RGPayWallView: View {
    ...
    // State management
    @State private var product: RGIAPManager.RGProduct?
    @State private var currentStatus: Int64 = 1
    @State private var state: PayWallState = .ready
    @State private var showErrorAlert: Bool = false
    @State private var showConfetti: Bool = false
    @State private var showDetails: Bool = false
    @State private var shownFeature: RGProFeatureId? = nil
    
    private let productsPub = NotificationCenter.default.publisher(for: .RGIAPHelperProductsUpdatedNotification, object: nil)
    private let buyOrRestoreSuccessPub = NotificationCenter.default.publisher(for: .RGIAPHelperPurchaseNotification, object: nil)
    private let buyErrorPub = NotificationCenter.default.publisher(for: .RGIAPHelperErrorNotification, object: nil)
    private let validationErrorPub = NotificationCenter.default.publisher(for: .RGIAPHelperValidationErrorNotification, object: nil)
    ...
    private var features: [RGProFeature]  {
        return [
    ...
    var body: some View {
        ZStack {
            ...
            ZStack(alignment: .bottom) {
                ...
                .padding(.leading, max(innerShadowWidth + 8.0, rgLeftSafeAreaInset(containerViewLayout)))
                .padding(.trailing, max(innerShadowWidth + 8.0, rgRightSafeAreaInset(containerViewLayout)))
                
                if showDetails {
                    RGPayWallFeatureDetails(
    ...
    private var purchaseSection: some View {
        VStack(spacing: 0) {
            ...
            .padding(.bottom, rgBottomSafeAreaInset(containerViewLayout) + 2.0)
        }
    ...
    private var buttonTitle: String {
        if currentStatus > 1 {
            ...
            if state == .purchasing {
                ...
                if !RGIAP.canMakePayments || paymentsEnabled == false {
    ...
    private func showDetailsForFeature(_ featureId: RGProFeatureId) {
        if #available(iOS 14.0, *) {
...
//        shownFeature = nil
    ...
    private func updateSelectedProduct() {
        product = RGIAP.availableProducts.first { $0.id == RG_CONFIG.iaps.first ?? "" }
    }
    ...
    private func handlePurchase() {
        if currentStatus > 1 {
            ...
            if Bundle.main.appStoreReceiptURL?.lastPathComponent == "sandboxReceipt" && !canBuyInBeta {
                ...
                state = .purchasing
                RGIAP.buyProduct(product.skProduct)
            }
        }
    }
