        ...
        |> deliverOnMainQueue).start(next: { [weak controller] view in
            ...
            func importNextBackup(index: Int) {
                ...
                if existingRecord != nil {
                    RGLogger.shared.log("SessionBackup", "Record \(backup.userId) already exists, skipping")
                ...
                let importBackupSignal = context.sharedContext.accountManager.transaction { transaction -> Void in
                    ...
                    RGLogger.shared.log("SessionBackup", "Imported record \(accountRecordId) for \(backup.userId)")
                    restoredSessions += 1
                }
                |> deliverOnMainQueue
        ...
        let controller = textAlertController(context: context, title: "SessionBackup.DeleteAll.Title".i18n(lang), text: "SessionBackup.DeleteAll.Text".i18n(lang), actions: [
            TextAlertAction(type: .destructiveAction, title: presentationData.strings.Common_Delete, action: {
                ...
                do {
                    ...
                    RGLogger.shared.log("SessionBackup", "Error deleting all sessions: \(e)")
                }
            }),
        ...
        let controller = textAlertController(context: context, title: "SessionBackup.DeleteSingle.Title".i18n(lang), text: "SessionBackup.DeleteSingle.Text".i18n(lang, args: "\(session.name ?? "\(session.userId)")"), actions: [
            TextAlertAction(type: .destructiveAction, title: presentationData.strings.Common_Delete, action: {
                ...
                do {
                    ...
                    RGLogger.shared.log("SessionBackup", "Error deleting session: \(e)")
                }
            }),
            TextAlertAction(type: .defaultAction, title: presentationData.strings.Common_Cancel, action: {})
        ])
...
//                }
        ...
        .onAppear {
            ...
            loggedInAccountsDisposable = accountsSignal.start(next: { view in
                ...
                RGLogger.shared.log("SessionBackup", "Logged in accounts: \(result)")
                if loggedInPeerIDs != result {
                    RGLogger.shared.log("SessionBackup", "Updating logged in accounts: \(result)")
    ...
    do {
        ...
        for sessionBackupData in backupSessionsData {
            do {
                ...
                RGLogger.shared.log("SessionBackup", "IMPORT ERROR: \(e)")
            }
        }
    } catch let e {
        RGLogger.shared.log("SessionBackup", "Error getting all sessions: \(e)")
    }
    return sessions
}
...
func backupSessionsFromView(_ view: AccountRecordsView<TelegramAccountManagerTypes>, accountsWithInfo: [AccountWithInfo] = []) {
    ...
    for (peerId, record) in recordsToBackup {
        ...
        do {
            ...
            RGLogger.shared.log("SessionBackup", "BACKUP ERROR: \(e)")
        }
    }
}
...
public func rgSessionBackupManagerController(context: AccountContext, presentationData: PresentationData? = nil) -> ViewController {
    ...
    let swiftUIView = RGSwiftUIView<SessionBackupManagerView>(
