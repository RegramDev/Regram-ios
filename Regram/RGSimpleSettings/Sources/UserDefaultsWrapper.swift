    ...
    public var wrappedValue: T {
        get {
            ...
            if let strongCachedValue = cachedValue {
                #if DEBUG && false
                RGtrace("UD", what: "CACHED \(key) \(strongCachedValue)")
                #endif
            ...
            #if DEBUG
            RGtrace("UD.\(key)", what: "EXTRACTED: \(cachedValue!)")
            #endif
            return cachedValue!
        }
        set {
...
public class UserDefaultsBackedDictionary<Key: Hashable, Value> {
    ...
    public var keys: [Key] {
        #if DEBUG
        RGtrace("UD.\(userDefaultsKey)\(threadSafe ? "-ts" : "")", what: "KEYS")
        #endif
        let result: [Key]
        if threadSafe {
            rwlock.readLock()
        ...
        if container == nil {
            ...
            #if DEBUG
            RGtrace("UD.\(userDefaultsKey)\(threadSafe ? "-ts" : "")", what: "EXTRACTED: \(container!)")
            #endif
        } else {
            #if DEBUG
            RGtrace("UD.\(userDefaultsKey)\(threadSafe ? "-ts" : "")", what: "FROM CACHE: \(container!)")
            #endif
        }
    ...
    public var values: [Value] {
        #if DEBUG
        RGtrace("UD.\(userDefaultsKey)\(threadSafe ? "-ts" : "")", what: "VALUES")
        #endif
        let result: [Value]
        if threadSafe {
            rwlock.readLock()
    ...
    public func contains(_ key: Key) -> Bool {
        #if DEBUG
        RGtrace("UD.\(userDefaultsKey)\(threadSafe ? "-ts" : "")", what: "CONTAINS")
        #endif
        let result: Bool
        if threadSafe {
            rwlock.readLock()
    ...
    public func value(forKey key: Key) -> Value? {
        #if DEBUG
        RGtrace("UD.\(userDefaultsKey)\(threadSafe ? "-ts" : "")", what: "VALUE")
        #endif
        let result: Value?
        if threadSafe {
            rwlock.readLock()
    ...
    public func mutateValue(forKey key: Key, mutation: (Value) -> Value) {
        #if DEBUG
        RGtrace("UD.\(userDefaultsKey)\(threadSafe ? "-ts" : "")", what: "MUTATE")
        #endif
        if threadSafe {
            rwlock.writeLock()
        }
        ...
        if let value = container![key] {
            ...
            #if DEBUG
            RGtrace("UD.\(userDefaultsKey)\(threadSafe ? "-ts" : "")", what: "UPDATING CACHE \(key): \(value), \(container!)")
            #endif
            userDefaultsContainer = container!
            #if DEBUG
            RGtrace("UD.\(userDefaultsKey)\(threadSafe ? "-ts" : "")", what: "CACHE UPDATED \(key): \(value), \(container!)")
            #endif
        }
        if threadSafe {
            rwlock.unlock()
    ...
    private func _remove(_ key: Key) -> Value? {
        ...
        #if DEBUG
        RGtrace("UD.\(userDefaultsKey)\(threadSafe ? "-ts" : "")", what: "UPDATING CACHE REMOVE \(key) \(container!)")
        #endif
        userDefaultsContainer = container!
        #if DEBUG
        RGtrace("UD.\(userDefaultsKey)\(threadSafe ? "-ts" : "")", what: "CACHE UPDATED REMOVED \(key) \(container!)")
        #endif
        return tuple.value
    }
    ...
    public func drop() {
        #if DEBUG
        RGtrace("UD.\(userDefaultsKey)\(threadSafe ? "-ts" : "")", what: "DROPPING")
        #endif
        if threadSafe {
            rwlock.writeLock()
        }
        ...
        container = userDefaultsContainer
        #if DEBUG
        RGtrace("UD.\(userDefaultsKey)\(threadSafe ? "-ts" : "")", what: "DROPPED: \(container!)")
        #endif
        if threadSafe {
            rwlock.unlock()
        }
...
public func RGtrace(_ domain: String, what: @autoclosure() -> String) {
    let string = what()
    var rawTime = time_t()
    time(&rawTime)
    var timeinfo = tm()
